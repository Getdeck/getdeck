...
def run(config: ClientConfiguration, cmd: Union[str, List], volume_mounts=None) -> str:
    ...
    content = config.DOCKER.containers.run(config.TOOLER_USER_IMAGE,
                                           exec_cmd,
...
def build_user_container(config: ClientConfiguration):
    ...
    Dockerfile = io.BytesIO((f"FROM {config.TOOLER_BASE_IMAGE} " + """
    ARG USER_ID
    ARG GROUP_ID
    ...
    image, build_logs = config.DOCKER.images.build(
        fileobj=Dockerfile, rm=True, forcerm=True, buildargs=build_args, tag=config.TOOLER_USER_IMAGE
    )
